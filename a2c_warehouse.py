...
import gym
from attention_module import DRRLnet, DRRLnetWarehouse
...
from warehouse_env.warehouse_env import PseudorandomWarehouseEnv
...
env = PseudorandomWarehouseEnv(10, 10, 4, 2)
N_ACT = 4
INP_W = 10
INP_H = 10
...
class Worker(mp.Process):
    def __init__(self, gnet, w_idx, device=l_device, verbose=False):
        ...
        self.g_net = gnet
        self.l_net = DRRLnetWarehouse(INP_W, INP_H, N_ACT).to(device)  # local network
        ...
        self.env = PseudorandomWarehouseEnv(10, 10, 4, 2)
        self.device = device
...
if __name__ == "__main__":
    ...
    g_net = DRRLnetWarehouse(INP_W, INP_H, N_ACT).to(g_device) # global network
